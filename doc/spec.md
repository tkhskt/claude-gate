...
│    ↓                                                  │
│  scripts/claude-menubar-hook.sh (curl, --max-time 75) │
└────────────────────────┬──────────────────────────────┘
                         │ POST JSON
                         ↓
...
| `lastTimeout: StateFlow<PermissionRequest?>` | 直近タイムアウトしたリクエスト（stick。`dismissTimeout` or 新リクエスト到着で解除）。トレイ色変更と TimeoutView 表示の唯一のトリガ |

公開メソッド: `submit / allow / deny / togglePopover / setPopoverVisible / dismissTimeout / resolveExternally`。
...
holder.submit(request)  [suspends]
  ...
  │     ├── 60s 経過 → TimeoutCancellationException
  │     │    └── _lastTimeout = request（トレイ色 → 赤、再展開で TimeoutView）
  ...
  ↓
exchange に 200+JSON（USER / EXTERNAL いずれも）/ 503（timeout）を書き戻す
```
...
| 値                                          | 所在 | デフォルト |
|---------------------------------------------|------|-----------|
| サーバー側 await 上限                       | `PermissionRequestHolder.DEFAULT_TIMEOUT_MS` | 60_000ms (60s) |
| hook script の curl `--max-time`            | `scripts/claude-menubar-hook.sh` | 75s |

- サーバーを 15s 早く切る設計: curl 側が socket を閉じる前にサーバーが 503 を返せる → mutex 解放が遅れない／IOException ログが出ない。
- なぜ 60s か: 通常パスは `PostToolUse`（allow 後に tool 実行）または `UserPromptSubmit`（次プロンプトで pending 放棄）のいずれかで早期解決されるが、ユーザーが popover を放置したまま新しいプロンプトも送らずに別作業をしたケースの最終救済として server timeout が動く。trade-off: 60s 以上考え込むユーザーが popover を見ながら判断する間に timeout が走ると Claude の標準プロンプトにフォールバックする。
- タイムアウト時に実行されること:
  - `_pending=null`, `_popoverVisible=false`（ポップオーバー自動クローズ）
  - `_lastTimeout = request`（再展開で TimeoutView、トレイ背景 amber → red）
